    ...
    public class TicketAdapter extends RecyclerView.Adapter<TicketAdapter.TicketViewHolder> {
        ...
        @Override
        public void onBindViewHolder(@NonNull TicketViewHolder holder, int position) { //gan du lieu vao tung item hien thi trong danh sach
            ...
            // phan nay phan biet staff va khach hang
            if (!isStaff) {
                if (ticket.getStatus().equalsIgnoreCase("Hủy")) {
                    holder.btnCancel.setEnabled(false);
                    holder.btnCancel.setAlpha(0.5f); //lam mo nut
                } else {
                    holder.btnCancel.setEnabled(true);
                    holder.btnCancel.setAlpha(1f);
                    holder.btnCancel.setOnClickListener(v -> {
                        if (listener != null) {
                            listener.onCancel(ticket); //gui su kien ve activity
                        }
                    });
                }
            }
            ...
            if (isStaff) {
                ...
                if (holder.btnConfirm != null && holder.tvStatus != null) {
                    if(holder.tvStatus.getText().toString().equals("Trạng thái: Đã xác nhận")) { //ấy bn ham confirm been db
        ...
        static class TicketViewHolder extends RecyclerView.ViewHolder { //tham chieu den cac view thanh phan trong recyclilerview
            ...
            public TicketViewHolder(@NonNull View itemView) { //giu cac view trong tung item hien thi trong danh sach
                super(itemView);
